...
const Admin: React.FC = () => {
  ...
  const getContractIssuers = async () => {
    try {
      // Query OneChain issuer registry contract
      ...
      // Parse issuer data from contract
      // For now, return empty data - will implement based on contract structure
  ...
  const getContractManagers = async () => {
    try {
      // Query OneChain admin contract for managers
      // For now, return empty data - will implement based on contract structure
      return { addresses: [], count: 0, metadata: {} };
    } catch (error) {
  ...
  const addIssuer = async () => {
    ...
    try {
      ...
      // Call OneChain contract to add issuer
      const tx = new Transaction();
      
      tx.moveCall({
        target: `${ONECHAIN_CONTRACTS.ADMIN}::admin::add_issuer`,
        arguments: [
          tx.pure.address(userForm.walletAddress),
  ...
  const toggleMarketplace = async () => {
    ...
    try {
      ...
      signAndExecuteTransaction(
        ...
        {
          onSuccess: (result: any) => {
            console.log('✅ Marketplace status updated!', result);
